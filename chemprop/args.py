...
class CommonArgs(Tap):
    '''If you want to use PDI-DMPNN+SME, please be sure to modify the parameters here!!
    如果想使用PDI-DMPNN+SME，请注意一定要修改这里的参数！！！'''
    #PDI-DMPNN+SME所在路径
    root_directory='/mnt/PDI-DMPNN+SME'
    #先验数据的数量
    number_of_prior_data: int = 17
    #prior data的路径
    solvent_filename:str=root_directory+'/prior_data_csv/normalize_solvent_sum2+implicit+HBA+HBD.csv'
#     solute_filename:str=root_directory+'/prior_data_csv/normalize_solute_sum2+guangkejiao+HBA+HBD.csv'
    solute_filename:str=root_directory+'/prior_data_csv/normalize_solute_sum2+soybean.csv'
    #PDI-DMPNN文件夹下有一个batch_number.json，这个文件的路径
    json_filename:str=root_directory+'/batch_number.json'
    #brics文件夹，存放sme掩蔽索引的路径
    sme_mask_npy_filename:str=root_directory+'/brics/solvent-brics_mol_graph_smask_for_brics.npy'
        
    #很重要，是否要进行sme掩蔽，train过程请务必改成False！
#     whether_sme_mask:bool=True
    whether_sme_mask:bool=False
    
    '''
    Here are the parameters that came with DMPNN before.
    下面是DMPNN之前的自带参数。
    '''
    ...
    smiles_columns: List[str] = ['mol solvent','mol solute']
    #smiles_columns: List[str] =['mol solvent']
...
#     checkpoint_path: str=root_directory+'/checkpoint_predict/model.pt'
#     checkpoint_path: str=root_directory+'/output/fold_0/model_0/model.pt'
...
class TrainArgs(CommonArgs):
    ...
    # General arguments
#     data_path: str
    data_path: str=CommonArgs.root_directory+'/data_csv/sampled_data1+soybean1.csv'
#     data_path: str=CommonArgs.root_directory+'/data_csv/sampled_data2+soybean2.csv'
    ...
    loss_function: Literal[
        'mse', 'bounded_mse', 'binary_cross_entropy', 'cross_entropy', 'mcc', 'sid', 'wasserstein', 'mve', 'evidential', 'dirichlet'] = 'mse'
    ...
    split_sizes: List[float] = [0.6,0.2,0.2]
...
#     save_dir: str = None
    save_dir: str = CommonArgs.root_directory+'/output'
    """Directory where model checkpoints will be saved."""
    checkpoint_frzn: str = None
#     checkpoint_frzn: str = CommonArgs.root_directory+'/checkpoint_predict/model.pt'
...
#     frzn_ffn_layers: int = 1
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        #看过来,这里加了一个对calculated_batch_prior_data的初始化
        self.calculated_batch_prior_data=None
...
#     test_path: str
    test_path: str=CommonArgs.root_directory+'/predict_csv/sampled_data1+soybean1.csv'
#     test_path: str=CommonArgs.root_directory+'/predict_csv/sampled_data2+soybean2.csv'
    """Path to CSV file containing testing data for which predictions will be made."""
#     preds_path: str
    preds_path: str=CommonArgs.root_directory+'/predict_csv/without_sampled_data1+soybean1.csv'
#     preds_path: str=CommonArgs.root_directory+'/predict_csv/without_sampled_data2+soybean2.csv'
