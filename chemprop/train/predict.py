...
#看过来，多引入了TrainArgs和 CommonArgs
from chemprop.args import TrainArgs,CommonArgs
#看过来，这里多引入了一个json类
import json

#看过来，在predict里面加一个TrainArgs
def predict(
    model: MoleculeModel,
    ...
    #看过来，这个batch_number是后加的
    batch_number=0
    
    for batch in tqdm(data_loader, disable=disable_progress_bar, leave=False):
        #看过来，这里传递了batch_number参数
        batch_number+=1
#         print('这里是第'+str(batch_number)+'个batch在运行中。')
        ...
        #看过来，这里是后加的
        from chemprop.train import batch_prior_data
        batch_smiles=batch.smiles()
        TrainArgs.calculated_batch_prior_data=batch_prior_data(batch_smiles)
