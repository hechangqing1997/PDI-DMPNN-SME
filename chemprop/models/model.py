...
from chemprop.args import CommonArgs,TrainArgs
...
class MoleculeModel(nn.Module):
    ...
    def __init__(self, args: TrainArgs):
        ...
        #看过来，这里是后加的
        # 在初始化方法中可以使用self.calculated_batch_prior_data来访问传递进来的值
#         self.calculated_batch_prior_data = calculated_batch_prior_data
#         print(calculated_batch_prior_data)
...
#             print('atom_first_linear_dim是：')
#             print(atom_first_linear_dim)
...
#             )
            ...
            #注意看，这里调整了ffn的输入，使其和分子+先验数据一样
            self.hidden_size=TrainArgs.hidden_size
            self.number_of_molecules=TrainArgs.number_of_molecules
            self.number_of_prior_data=CommonArgs.number_of_prior_data
    
            ffn_input=self.hidden_size*self.number_of_molecules+self.number_of_prior_data

            self.readout = build_ffn(
                first_linear_dim=ffn_input,
...
#             print('看过来，之前encodings的形状为:')
#             print(encodings.shape)
#             print('看过来，TrainArgs.calculated_batch_prior_data为:')
#             print(TrainArgs.calculated_batch_prior_data)

            calculated_batch_prior_data=torch.tensor(TrainArgs.calculated_batch_prior_data)
#             print('encodings的行数为：')
#             print(encodings.size(0))
#             print('calculated_batch_prior_data的行数为：')
#             print(calculated_batch_prior_data.size(0))
            assert encodings.size(0) == calculated_batch_prior_data.size(0), "The number of rows in encoding and prior_data should be the same."
            # 将calculated_batch_prior_data移动到与encodings相同的设备上
            calculated_batch_prior_data = calculated_batch_prior_data.to(encodings.device)
            concatenated_tensor = torch.cat((encodings, calculated_batch_prior_data), dim=1)
            encodings=concatenated_tensor
            
#             print('看过来，改正后encodings的形状为:')
#             print(encodings.shape)
