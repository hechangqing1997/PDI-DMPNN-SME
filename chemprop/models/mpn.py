...
#看过来，这里加函数了
import json
from chemprop.models.sme_mask import sme_mask
...
#                  bias: bool = None, depth: int = None):
    ...
    #注意，这里修改了，添加了check_sme参数
    def __init__(self, args:TrainArgs, atom_fdim: int, bond_fdim: int, hidden_size: int = None,
                 bias: bool = None, depth: int = None,check_sme: bool=None):
        """
       ...
       """
        ...
        #注意，这里check_sme进行初始化了
        self.check_sme=check_sme
    ...
    def forward(self,
        ...
        #看过来，这里在读取batch_number
        json_filename=CommonArgs.json_filename
        with open(json_filename, 'r') as file:
            data = json.load(file)
        batch_number = int(data.get('batch_number'))
        
        if self.check_sme and CommonArgs.whether_sme_mask:
            print('注意，进行了sme掩蔽操作！')
            #看过来，这里是掩蔽的重要步骤
            a_message=sme_mask(a_message,batch_number)
        elif self.check_sme and not CommonArgs.whether_sme_mask:
            print('注意，没有进行sme掩蔽操作！')
...
#                                              for _ in range(args.number_of_molecules)])
                self.encoder = nn.ModuleList([MPNEncoder(args, self.atom_fdim, self.bond_fdim, check_sme=(m == 0))for m in range(args.number_of_molecules)])
        else:
...
#                 print(encodings[0][0].shape)
#                 print(type(encodings[0][0]))
            else:
                encodings = []
                for ba in batch:
                    if ba.is_reaction:
